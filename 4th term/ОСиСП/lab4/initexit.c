...
void init(void)
{
    ...
    void *ptr = mmap(NULL, sizeof(msg_queue), PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    if (ptr == MAP_FAILED)
    {
        perror("mmap");
        exit(errno);
    ...
    // Setup queue with zeroes
    msg_queue_init();
    // Setup semaphores
    if ((mutex = sem_open(MUTEX, OPEN_FLAGS, MODE, 1)) == SEM_FAILED ||
        (free_space = sem_open(FREE_SPACE, OPEN_FLAGS, MODE, MSG_MAX)) == SEM_FAILED ||
