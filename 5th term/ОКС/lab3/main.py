...
class SerialApp:
    ...
    def read_serial(self):
        while self.running:
            if self.serial2.in_waiting > 0:
                ...
                data_content = self.hamming_decode(data_content, data_decoded[18:])
    ...
    def corrupt_data(self, data):
        self.update_debug(f"Original data {data}")
        data_array = list(data)
    ...
    def update_status(self, frame):
        ...
        self.status_text.insert(tk.END, text[:space] + " " + text[space:])
        self.status_text.tag_config("highlight", background="yellow")

        for i in index:
            self.status_text.tag_add("highlight", f"1.0 + {i} chars", f"1.0 + {i + 1} chars")
